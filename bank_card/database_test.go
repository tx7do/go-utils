...
func TestImportBankCardName(t *testing.T) {
	db := NewDatabase(true)
	defer db.Close()
	...
	for k, v := range bankCode {
		bin, _ := strconv.Atoi(k)
		strs := strings.Split(v, "-")
		if len(strs) == 3 {
			db.UpdateBankCardTableCardName(uint32(bin), strs[1])
		}
	}
}
...
func TestOpenFromEmbed(t *testing.T) {
	...
	assert.Equal(t, bankCard.CardName, "预付卡")
